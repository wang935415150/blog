...
def articleDetail(request,user_site,article_id):
    ...
    commnet_list=Comment.objects.all()

    print("coment_list",commnet_list) # [obj,obj]

    commnet_list2=Comment.objects.values("nid","content","parent_id_id")

    print("coment_list2", commnet_list2)
...
def articleDetail2(request,user_site,article_id):
    ...
    if request.is_ajax():
        comment_list=Comment.objects.filter(article_id=article_id).values("nid","content","parent_id_id")
    ...
    return render(request,"articleDetail2.html",locals())
